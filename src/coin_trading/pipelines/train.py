...
def _required_data_gaps(train_rows: int, val_rows: int, test_rows: int, cfg: AppConfig) -> dict[str, dict[str, int]]:
    gaps: dict[str, dict[str, int]] = {}
    if train_rows < cfg.train.min_train_rows:
        gaps["train"] = {"required": int(cfg.train.min_train_rows), "actual": int(train_rows)}
    if val_rows < cfg.train.min_val_rows:
        gaps["val"] = {"required": int(cfg.train.min_val_rows), "actual": int(val_rows)}
    if test_rows < cfg.train.min_test_rows:
        gaps["test"] = {"required": int(cfg.train.min_test_rows), "actual": int(test_rows)}
    return gaps
...
def run() -> str:
    ...
    split_gaps = _required_data_gaps(len(train_df), len(val_df), len(test_df), cfg)
    if dataset_summary["rows"] > 0 and split_gaps:
        status = "blocked_insufficient_split_data"
        train_summary = {
            "enabled": False,
            "reason": "insufficient_split_data",
            "required_rows": split_gaps,
            "message": "Increase split coverage or reduce train.min_*_rows thresholds.",
        }
    elif dataset_summary["rows"] > 0:
        try:
    ...
    if status == "blocked_missing_dependencies":
        missing_items.append("stable-baselines3/gymnasium dependencies")
    if status == "blocked_insufficient_split_data":
        missing_items.append("minimum split rows threshold")
    ...
    write_train_manifest(
        run_dir,
        {
            ...
            "required_min_rows": {
                "train": int(cfg.train.min_train_rows),
                "val": int(cfg.train.min_val_rows),
                "test": int(cfg.train.min_test_rows),
            },
            "epochs": 0,
