...
def test_train_run_writes_dependency_block_or_training_artifacts():
    ...
    assert train_manifest["status"] in {"ready", "blocked_missing_dependencies", "blocked_insufficient_split_data"}
    ...
    elif train_manifest["status"] == "blocked_missing_dependencies":
        assert model_train["enabled"] is False
        assert model_train["reason"] == "missing_dependencies"
    else:
        assert model_train["enabled"] is False
        assert model_train["reason"] == "insufficient_split_data"
        assert "required_rows" in model_train
